* THIS PROGRAM CREATES TABLES B6 top panel IN AUTOR, PALMER, PATHAK (JPE, 2013)
...
rename state_use_cd class02
gen house02 = inlist(class02,101,104,105)
gen condo02 = class02==199
gen apt02 = inlist(class02,111,112)
drop if !inlist(class02,101,104,105,111,112,199)
duplicates drop
duplicates report ml
keep ml house02 condo02 apt02
sort ml 
tempfile ml02
save `ml02', replace

use ../dta/assess_vol1234, clear
ren equiv class
gen house94 = inlist(class,101,104,105)
gen condo94 = class==102
gen apt94=inlist(class,111,112)
ren class usecode
keep ml house94 condo94 apt94 usecode
sort ml
merge ml using `ml02', uniqus nokeep
gen goodflag=0
replace goodflag = 1 if house02==1 | condo02==1 | apt02==1
replace goodflag = 1 if inlist(usecode,102,101,104,105,111,112,199)
replace goodflag = 0 if inlist(usecode,121,304)
drop if goodflag == 0
duplicates drop
duplicates report ml
ren usecode usecode94
replace house94=1 if house02==1 & !inlist(usecode94,102,101,104,105,111,112,199)
replace condo94=1 if condo02==1 & !inlist(usecode94,102,101,104,105,111,112,199)
replace apt94=1 if apt02==1 & !inlist(usecode94,102,101,104,105,111,112,199)
assert apt94+house94+condo94 == 1
drop house02 condo02 apt02 _merge

/*manually drop remaining duplicates*/

bys ml: gen check=_N
gen dupflag=0
replace dupflag=1 if usecode!=102 & check>1
replace dupflag=0 if usecode==339
drop if dupflag==1
duplicates report ml
drop dupflag goodflag check
sort ml
tempfile converts-transcations
save `converts-transactions'
...
sort ml
merge ml using `converts-transactions', uniqusing nokeep
tab usecode usecode94
keep if usecode==usecode94
...
outreg2 `rc_control' `rcivars' using "tab/table-b6a", excel replace bdec(3) tdec(3) ///
...
local zero = r(p)
outreg2 `rc_control' `rc_rcivars' using "tab/table-b6a", excel append bdec(3) tdec(3) ///
...
** #############################
** 3diff - by struct type
** #############################

local app = "replace"
local rc_rcivars "nrc_rci p_nrc_rci rc_rci prc_rci"
foreach Group in "!condo" "condo" {
	...
	* 2diff first
	areg lprice `rc_control' rci p_rci `struct_type' `interacted_exis' year19* year20* if `Group', cluster(clusvar) a(`fe')
	outreg2 p_rc p_rci using ../tab/table-b6a-struct, excel `app' bdec(3) tdec(3) /// 
		addtext(Group,`Group',Other X's,x, Fixed Effects, y, Trend,-) nocons label nonotes ctitle(rci`main_rad')
	local app "append"
	

	areg lprice `rc_control' `rc_rcivars' `struct_type' `interacted_exis' year19* year20* if `Group', cluster(clusvar) a(`fe')
	test p_nrc_rci = prc_rci
	local equal = r(p)
	test p_nrc_rci = prc_rci = 0
	local zero = r(p)
	outreg2  p_rc p_nrc_rci prc_rci ///
		using ../tab/table-b6a-struct, title(3diff by struct type) excel append bdec(3) tdec(3) /// 
		addstat(Eq and Zero, `zero',Equal, `equal'  ) addtext(Group,`Group',Other X's,x, Fixed Effects, `e(absvar)', Trend,-) nocons label nonotes ctitle(rci`main_rad')

	areg lprice `rc_control' `rc_rcivars' `struct_type' `interacted_exis' `trendvars' year19* year20* if `Group', cluster(clusvar) a(`fe')
	test p_nrc_rci = prc_rci
	local equal = r(p)
	test p_nrc_rci = prc_rci = 0
	local zero = r(p)
	outreg2 p_rc p_nrc_rci prc_rci  ///
		using ../tab/table-b6a-struct, excel append bdec(3) tdec(3) /// 
		addstat(Eq and Zero, `zero',Equal, `equal'  ) addtext(Group,`Group',Other X's,x, Fixed Effects, `e(absvar)', Trend,`1') nocons label nonotes ctitle(rci`main_rad')
